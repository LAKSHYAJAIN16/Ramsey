# Ramsey — historical setup and API notes

> Preserved from the earlier README. Current project layout, test results and device status are in [README.md](README.md) and [docs/build-plan.md](docs/build-plan.md). In particular, `unity-client/` is now a complete openable project rather than only a source mirror.
...
The native client source, bundled offline recipe, and build/hosting instructions
are now in [`unity-client/`](unity-client/README.md). The installed Quest app
runs independently of the laptop; online AI features connect to a configurable
backend URL. A [`Dockerfile`](Dockerfile) prepares the backend for hosting.
